...
export function DataEntryManager() {
  ...
  const form = useForm<FormValues>({ resolver: zodResolver(dataEntrySchema), defaultValues: { date: new Date().toISOString().slice(0, 10), brand: 'WINHOME', source: 'FACEBOOK', count: undefined } });
  const listQuery = useQuery({ queryKey: ['data-entries', queryString], queryFn: () => apiFetch<{ items: Entry[] }>(`/api/data-entries${queryString}`) });
  const saveMutation = useMutation({ mutationFn: (values: FormValues) => editing ? apiFetch(`/api/data-entries?id=${editing.id}`, { method: 'PUT', body: JSON.stringify(values) }) : apiFetch('/api/data-entries', { method: 'POST', body: JSON.stringify(values) }), onSuccess: () => { toast.success(editing ? 'Cập nhật thành công' : 'Thêm mới thành công'); setEditing(null); form.reset({ date: new Date().toISOString().slice(0, 10), brand: 'WINHOME', source: 'FACEBOOK', count: undefined }); queryClient.invalidateQueries({ queryKey: ['data-entries'] }); }, onError: (error: Error) => toast.error(error.message) });
