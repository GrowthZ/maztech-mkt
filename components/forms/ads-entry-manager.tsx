...
export function AdsEntryManager() {
  ...
  const form = useForm<FormValues>({ resolver: zodResolver(adsEntrySchema), defaultValues: { date: new Date().toISOString().slice(0, 10), brand: 'WINHOME', spend: undefined, messages: undefined, data: undefined } });
  const listQuery = useQuery({ queryKey: ['ads-entries', queryString], queryFn: () => apiFetch<{ items: Entry[] }>(`/api/ads-entries${queryString}`) });
  const saveMutation = useMutation({ mutationFn: (values: FormValues) => editing ? apiFetch(`/api/ads-entries?id=${editing.id}`, { method: 'PUT', body: JSON.stringify(values) }) : apiFetch('/api/ads-entries', { method: 'POST', body: JSON.stringify(values) }), onSuccess: () => { toast.success(editing ? 'Cập nhật thành công' : 'Thêm mới thành công'); setEditing(null); form.reset({ date: new Date().toISOString().slice(0, 10), brand: 'WINHOME', spend: undefined, messages: undefined, data: undefined }); queryClient.invalidateQueries({ queryKey: ['ads-entries'] }); }, onError: (error: Error) => toast.error(error.message) });
