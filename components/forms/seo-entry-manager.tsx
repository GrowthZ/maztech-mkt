...
import { OWNER_OPTIONS, WEBSITE_OPTIONS } from '@/lib/constants';
...
import { formatOwnerLabel } from '@/lib/presentation';
import { ownerNameFromIdentity } from '@/lib/permissions';

type FormValues = z.infer<typeof seoEntrySchema>;
...
export function SeoEntryManager() {
  ...
  const form = useForm<FormValues>({
    resolver: zodResolver(seoEntrySchema),
    defaultValues
  });
  ...
  const listQuery = useQuery({ queryKey: ['seo-entries', queryString], queryFn: () => apiFetch<{ items: Entry[] }>(`/api/seo-entries${queryString}`) });

  const saveMutation = useMutation({
    mutationFn: (values: FormValues) => editing ? apiFetch(`/api/seo-entries?id=${editing.id}`, { method: 'PUT', body: JSON.stringify(values) }) : apiFetch('/api/seo-entries', { method: 'POST', body: JSON.stringify(values) }),
    onSuccess: () => { toast.success(editing ? 'Cập nhật thành công' : 'Thêm mới thành công'); setEditing(null); form.reset(defaultValues); queryClient.invalidateQueries({ queryKey: ['seo-entries'] }); },
    onError: (error: Error) => toast.error(error.message)
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiFetch(`/api/seo-entries?id=${id}`, { method: 'DELETE' }),
    onSuccess: () => { toast.success('Xóa thành công'); queryClient.invalidateQueries({ queryKey: ['seo-entries'] }); },
    onError: (error: Error) => toast.error(error.message)
  });
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[420px,1fr]">
      <Card>
        <CardTitle className="mb-4">{editing ? 'Sửa bài SEO' : 'Nhập bài SEO theo ngày'}</CardTitle>
        <div className="mb-4 grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl bg-slate-50 p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Nguyên tắc nhập liệu</div>
            <div className="mt-2 text-sm font-medium text-slate-900">Website chọn sẵn từ danh mục</div>
            <div className="mt-1 text-sm leading-6 text-slate-600">Không để nhân sự gõ tay tên website để tránh phát sinh dữ liệu rác khi tổng hợp báo cáo.</div>
          </div>
          <div className="rounded-2xl bg-slate-50 p-4">
            <div className="text-xs uppercase tracking-wide text-slate-500">Mục tiêu</div>
            <div className="mt-2 text-sm font-medium text-slate-900">SEO tách riêng, nhìn rõ sản lượng</div>
            <div className="mt-1 text-sm leading-6 text-slate-600">Phân tách website và nhân sự ngay từ đầu giúp báo cáo SEO sạch và dễ đọc hơn.</div>
          </div>
        </div>
      ...
      <Card>
        <CardTitle className="mb-4">Danh sách SEO</CardTitle>
        {listQuery.data?.items.length ? <div className="overflow-x-auto"><Table><THead><TR><TH>Ngày</TH><TH>Nhân sự</TH><TH>Website</TH><TH>Số bài</TH><TH></TH></TR></THead><TBody>{listQuery.data.items.map((item, index) => <TR key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'}><TD>{formatDate(item.date)}</TD><TD>{formatOwnerLabel(item.ownerName)}</TD><TD>{item.website}</TD><TD>{item.quantity}</TD><TD className="space-x-2"><Button variant="outline" onClick={() => { setEditing(item); form.reset({ ...item, date: item.date.slice(0, 10) }); }}>Sửa</Button><Button variant="danger" onClick={() => { if (window.confirm('Xóa bản ghi?')) deleteMutation.mutate(item.id); }}>Xóa</Button></TD></TR>)}</TBody></Table></div> : <EmptyState />}
      </Card>
    </div>
  );
}
